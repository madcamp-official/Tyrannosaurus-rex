...
## 2026-07-29 - 자이로 좌우 조준 단순화

- 구현 목적: 화면 방향 보정 후 자이로 조준이 기기별로 부자연스럽게 움직이는 문제를 제거했다.
- 주요 변경 사항: 화면 회전각에 따라 `beta`와 `gamma`를 혼합하던 처리를 제거하고, 휴대폰 자체의 세로축 회전값인 `gamma`만 좌우 조준에 사용하도록 복원했다. 운석 생성량 증가는 그대로 유지했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: Frontend 테스트 5개 및 전체 프로덕션 빌드 통과.

## 2026-07-29 - 화면 세로축 기준 자이로 조준

- 구현 목적: 휴대폰의 세로축을 중심으로 좌우로 비트는 동작이 화면 방향과 관계없이 조준점의 좌우 이동으로 이어지게 했다.
- 주요 변경 사항: `screen.orientation.angle`을 기준으로 `beta`와 `gamma`를 현재 화면 좌표계로 변환한다. 세로 화면에서는 `gamma`, 가로 화면에서는 회전 방향에 맞게 `beta`가 좌우 조준을 담당한다. 화면 가로축 기준 상하 조준도 함께 보정했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: Frontend 테스트 5개, Backend 테스트 62개 및 전체 프로덕션 빌드 통과.
...
## 2026-07-29 - 사격 티라노 카메라 축 교정 및 화석 자유 회전

- 구현 목적: 사격 화면에서 스켈레톤 티라노가 거의 보이지 않던 카메라 축 문제를 해결하고 발굴·조립 화석 자세를 다양화한다.
- 주요 변경 사항:
  - GLTF의 머리~꼬리 Z축을 화면 가로축으로 90도 회전해 정면에서 전체 실루엣이 보이게 한다.
  - 모델 깊이 최댓값 대신 실제 투영 가로·세로 크기로 카메라 거리를 계산하고 렌더 캔버스를 620×360으로 확대한다.
  - 뼈 재질을 양면 렌더링해 얇은 골격 면이 카메라 방향에 따라 사라지지 않게 한다.
  - 흩어진 화석 조각의 X·Z 회전 범위를 ±0.35에서 ±1.1라디안으로 넓혀 다양한 각도로 나타나게 한다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: GLTF 운영 자산을 실제 파싱해 254개 메시·48,667개 정점을 확인했고 교정 후 캔버스 가로 약 84.85%, 세로 약 48.81%를 차지함을 계산했다. `npm run typecheck`, `npm test`(72개), Godot 4.7.1 Web export, `npm run build`, `git diff --check` 통과.

## 2026-07-29 - 운석 팀 카드 한 줄 레이아웃 보정

- 구현 목적: 긴 팀명과 플레이어 정보가 카드 안에서 세로로 깨지거나 줄을 넘는 문제를 해결한다.
- 주요 변경 사항:
  - 팀명 길이에 따라 16~32px 범위에서 글자 크기를 자동 계산하고 한 줄 말줄임을 적용한다.
  - 플레이어 행 글자·간격·패딩을 카드 폭에 맞춰 반응형으로 축소한다.
  - 닉네임에만 유동 폭과 말줄임을 허용하고 점수·생명·탈락 표시는 한 줄 고정 폭으로 유지한다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: `npm run typecheck`, `npm test`(72개), `npm run build`, `git diff --check` 통과.
