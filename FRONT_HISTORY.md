...
## 2026-07-30 - 독립 자이로 테스트 화면

- 구현 목적: 전체 게임을 진행하지 않고 휴대폰에서 실제 사격과 동일한 자이로 조작을 바로 시험할 수 있게 했다.
- 주요 변경 사항: `/gyro-test` 독립 경로를 추가했다. 기존 `AimControls`가 소켓 없이도 연습 모드로 동작하도록 확장해 센서 계산을 복제하지 않고 본 게임과 정확히 같은 방향·감도·필터·데드존을 사용한다. 센서 연결, 영점 여부, 실시간 X/Y 좌표도 표시한다.
