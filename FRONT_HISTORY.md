...
## 2026-07-30 - 자이로 조준 반응성 개선

- 구현 목적: 자이로 조준이 둔하고 원하는 방향으로 충분히 이동하지 않는 조작 문제를 개선했다.
- 주요 변경 사항: 화면 끝 도달 각도를 좌우 100도에서 45도, 상하 40도에서 35도로 조정했다. 센서 필터의 새 입력 반영률을 50%에서 75%로 높이고, 1도 데드존을 별도로 적용해 반응 속도를 높이면서 미세 떨림을 억제했다. 기존 좌우·상하 방향은 유지했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: Frontend 테스트 5개 및 전체 프로덕션 빌드 통과.

## 2026-07-30 - 사격 장면 이중 WebGL 제거

- 구현 목적: 1차 최적화 후에도 남은 사격 장면의 심한 렉을 제거했다.
- 주요 변경 사항: `CHARGING` 동안 뒤에서 실행되던 Godot iframe을 언마운트해 Three.js와의 이중 WebGL 렌더링을 없앴다. 254개 메시의 티라노는 매 프레임이 아닌 로드 시 한 번만 렌더링하고, 실제 이동은 기존 DOM 컨테이너가 담당한다. 조준 갱신을 15Hz로 제한하고 70ms CSS 보간을 추가했다.
- API/Socket.IO/Shared 계약 변경: 이벤트 구조 변경 없이 `AIM_UPDATE_MAX_HZ`를 30에서 15로 조정했다.
- 환경 변수 변경: 없음.
- 검증 결과: Frontend 테스트 5개, Backend 테스트 62개 및 전체 프로덕션 빌드 통과. 실제 운영 기기의 체감 프레임은 배포 후 추가 확인이 필요하다.

## 2026-07-30 - 사격 장면 렌더링 성능 개선

- 구현 목적: 사격 장면에서 Godot과 Three.js가 동시에 고부하 렌더링하며 발생하는 심한 프레임 저하를 완화했다.
- 주요 변경 사항: Three.js 티라노 캔버스의 픽셀 비율을 1로 고정하고 미세 애니메이션을 최대 30fps로 제한했다. 움직이는 WebGL 캔버스에 적용되던 중첩 CSS `drop-shadow` 필터를 제거했다. React 배틀 화면이 조준점을 표시하는 `CHARGING` 단계에는 동일 좌표를 Godot으로 중복 전송하지 않도록 했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: Frontend 테스트 5개 및 전체 프로덕션 빌드 통과. 실제 운영 기기의 체감 프레임은 배포 후 추가 확인이 필요하다.

## 2026-07-29 - 자이로 좌우 방향 반전

- 구현 목적: 휴대폰 오른쪽 가장자리를 몸쪽으로 비틀었을 때 조준점이 반대로 움직이는 문제를 수정했다.
- 주요 변경 사항: 좌우 조준에 사용하는 `gamma` 변화량의 부호를 반전해 오른쪽 가장자리를 몸쪽으로 비틀면 조준점도 오른쪽으로 이동하게 했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: Frontend 테스트 5개 및 전체 프로덕션 빌드 통과.
...
- 환경 변수 변경: 없음.
- 검증 결과: `npm run typecheck`, `npm test`(72개), Godot 4.7.1 Web export, `npm run build`, `git diff --check` 통과.
## 2026-07-29 — 사격 티라노 2배 확대 및 렌더링 부하 절감

- 사격 화면의 티라노 표시 영역을 `620×360`에서 `1240×720`으로 2배 확대했다.
- 254개 메시를 매 프레임 렌더링하는 비용을 줄이기 위해 티라노 WebGL 캔버스의 DPR을 1로 고정하고, 안티앨리어싱을 끄고, 렌더링을 30fps로 제한했다.
- 중복으로 적용되던 CSS `drop-shadow`를 하나 제거해 매 프레임 합성 비용을 줄였다.
## 2026-07-29 — 승패 기준 티라노사우루스·와이라노 결과 연출

- `PLAN.md`의 최종 형태 판정을 부활 성공 여부가 아닌 3경기 누적 점수 승패 기준으로 변경했다.
- 승리 팀 결과 슬롯에는 경량 육체 티라노 3D 모델이 좌우로 폴짝거리며 이동하는 연출을 추가했다.
- 패배 팀은 부활 상태와 무관하게 와이라노로 표시하고, 정지 렌더링으로 추가 GPU 부하를 억제했다.
- 무승부는 양 팀 모두 와이라노로 표시한다.
- 결과용 육체 티라노는 `model/source/trex` 원본 메시와 피부 텍스처를 사용한다.
- 원본 35.55MB 자산은 16,129개 정점과 28,152개 삼각형을 유지하면서 텍스처를 1024px WebP로 줄이고 단일 GLB로 묶어 902KB까지 경량화했다.
## 2026-07-29 — 결과 티라노 시선 변화 및 와이라노 확대

- 승리 티라노가 정면·오른쪽·왼쪽을 번갈아 보도록 결과용 3D 모델의 Y축을 12fps로 회전시켰다.
- 폴짝임과 좌우 이동은 기존 CSS 애니메이션이 담당하고, 탭이 보이지 않을 때는 3D 렌더링을 건너뛰어 부하를 제한했다.
- 결과 팀 카드는 최대 480px 너비, 최소 280px 높이로 조정하고 와이라노 캔버스를 최대 440×256px로 확대해 카드에 맞췄다.
## 2026-07-29 — Android 강제 데스크톱 viewport 모바일 UI 대응

- 일부 Android 브라우저가 약 980px의 데스크톱 viewport를 사용해 `max-width: 640px` 모바일 스타일을 건너뛰던 문제를 수정했다.
- 화면 폭뿐 아니라 호환성이 높은 `any-pointer: coarse` 조건으로 터치 기기를 감지해 로고·팀명·준비 버튼·게임 조작 UI를 큰 모바일 규격으로 표시한다.
- 터치 기기에서는 무거운 배경 사진 대신 기존 모바일 팀 컬러 배경을 적용한다.
## 2026-07-29 - 모바일 입장·발굴 UI와 참가자 재접속

- 구현 목적: 모바일 입장 폼을 과도하게 키운 이전 보정값의 약 60%로 줄이고, 발굴 화면의 시각 요소와 완료 안내를 팀 UI 스타일로 통일했다.
- 주요 변경 사항: 닉네임 입력창과 입장 버튼을 320px 카드 안의 44px 높이로 조정했다. 뼈 이모지를 제거하고 CSS 애니메이션을 유지하는 인라인 삽 SVG로 교체했으며, 파기 버튼과 발굴 완료 문구에 `YeogiOttaeJalnan` 글꼴을 적용하고 완료 문구를 확대했다.
...
## 2026-07-29 - 레이저건 중앙 대칭 조준

- 구현 목적: 사격 화면의 두 레이저건이 양옆에서 각각 `/`, `\` 형태로 중앙을 동시에 조준하도록 시각 방향을 통일했다.
- 주요 변경 사항: 좌우 총의 회전 기준점을 하단 중앙으로 고정하고, 좌측은 시계 방향 16도, 우측은 반시계 방향 16도로 동일하게 회전시켜 대칭 구도를 만들었다. 총구 섬광도 같은 컨테이너 안에서 함께 회전하므로 발사 시 조준 방향과 일치한다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 발굴 HUD 상하 배치 정리

- 구현 목적: 화석 발굴 중 진행 상태를 먼저 확인하고 기여도 정보는 화면 아래에서 확인할 수 있도록 HUD의 정보 우선순위를 조정했다.
- 주요 변경 사항: 다음 뼈 발굴 진행 바를 화면 위쪽으로 옮기고 기여도 패널을 아래쪽으로 내렸다. 진행 바 위의 `다음 뼈까지` 문구는 제거하고 진행률 숫자만 유지했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 사격 모델 구도와 결과 3D 무대 개선

- 구현 목적: 레이저건을 동일한 후방 상단 구도의 좌우 대칭으로 표시하고, 사격 티라노의 바닥 밀착감과 화질 및 결과 화면의 3D 공간감을 개선했다.
- 주요 변경 사항: A팀 레이저건의 참고 구도를 기준으로 B팀은 동일 렌더를 수평 반전하도록 변경했다. 사격 티라노는 하단 기준선을 따라 이동하게 하고 표시 크기와 내부 렌더 해상도를 각각 두 배로 높였다. 런타임 배경 생성 대신 고해상도 사전 제작 배경을 사용해 GPU 부하를 줄였다. 승리 티라노 장면에는 밤하늘, 텍스처가 적용된 3D 초원 바닥, 그림자와 조명을 추가했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 승리 티라노 정면 점프 및 밤 초원

- 구현 목적: 결과 화면의 티라노가 빙글거리며 걷는 동작과 잘린 초원 배경을 정리했다.
- 주요 변경 사항: 육체 티라노의 좌우 이동과 Y축 회전을 제거하고 카메라 정면을 유지한 채 제자리에서 반복 점프하도록 변경했다. 결과 화면 전체 지면에는 발굴 화면과 동일한 잔디 텍스처를 사용하고, 상단 하늘에는 기존 Godot 밤하늘 텍스처를 적용해 작은 3D 캔버스 경계에서 초원이 잘려 보이지 않게 했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 카운트다운 사운드·화석 원호 배치·티라노 보행 연결감

- 구현 목적: 페이즈 시작을 소리로 인지시키고, 발굴 화석 배치와 사격 티라노 움직임의 시각적 연결감을 개선했다.
- 주요 변경 사항: 데스크탑과 모바일의 서버 카운트다운 숫자가 바뀔 때 짧은 비프음을 내고 시작 순간에는 상승음을 재생한다. 발굴 화석은 구덩이 맞은편인 화면 위쪽 반원에 반경 편차를 두고 집중 배치한다. 사격 티라노는 정면에 오래 머무는 연속적인 `sin³` 회전 곡선을 사용하며 최대 회전각과 Y축 보행 흔들림을 줄였다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: Godot Web export, 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 레이저건 3/4 조준 시점 및 결과 KAIST 무대

- 구현 목적: 레이저건을 참고 이미지처럼 비스듬히 내려다보는 시점으로 표현하고 최종 결과 화면을 사격 무대와 연결했다.
- 주요 변경 사항: 좌우 Retrogun 카메라를 상단 3/4 시점으로 낮추고 모델 회전을 거울 대칭으로 적용했다. 화면 회전각을 ±28도로 넓혀 두 총구가 KAIST 교문 방향인 화면 상단 중앙으로 모인다. 결과 화면 전체에는 사격의 KAIST 석양 배경을 블러·감광 처리해 사용하며, 승리 티라노 렌더러의 자체 하늘과 초원 평면을 제거해 실제 육체 3D 모델이 배경 위를 투명하게 돌아다닌다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 개인 MVP 시상대 순위 배치

- 구현 목적: 개인 MVP 순위를 시상대처럼 1위가 위쪽 중앙, 2위와 3위가 아래쪽 좌우에 보이도록 정리했다.
- 주요 변경 사항: MVP 목록을 상위 3명으로 제한하고 첫 번째 항목이 전체 열을 차지한 상태로 중앙에 오도록 배치했다. 2명만 있을 때는 두 번째 항목도 아래 중앙에 놓여 레이아웃 균형을 유지한다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 탑다운 발굴 화석 가독성 개선

- 구현 목적: 현재 수직 탑다운 시점을 유지하면서 발굴된 화석이 얇은 선처럼 보이거나 흙에 묻혀 보이는 문제를 개선했다.
- 주요 변경 사항: 흩어진 화석의 X/Z 기울기 범위를 약 ±22도로 제한해 넓은 면이 카메라를 향하도록 하고, 지면 높이 편차를 줄여 구덩이에 가려지지 않게 했다. 발굴 단계에서만 따뜻한 아이보리색의 약한 발광 오버레이를 적용하고 조립 단계 진입 시 제거해 원래 재질로 돌아간다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: Godot Web export, 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 결과 화면 최종 승부 박스 및 점선 제거

- 구현 목적: 최종 결과 화면의 불필요한 중앙 박스와 점선 테두리를 없애고 와이라노 이미지와 점수 카드의 폭을 정렬했다.
- 주요 변경 사항: 가운데 `최종 승부` 슬롯을 마크업에서 제거하고 팀 결과 슬롯의 점선과 반투명 배경을 모두 없앴다. 결과 슬롯과 하단 팀 점수 카드가 동일한 480px 그리드 폭을 사용하며, 와이라노 이미지는 슬롯의 전체 폭을 채우면서 원본 비율을 유지한다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 사격 티라노 정면 등장 및 시점 변화

- 구현 목적: 사격 화면의 티라노가 옆모습으로만 보이지 않도록 정면으로 등장시키고, 이동 중 여러 각도의 실루엣을 보여 준다.
- 주요 변경 사항: 스켈레톤 티라노의 최초 회전을 정면 자세로 바꿨다. 이후 15fps 제한 애니메이션에서 좌우 회전과 가벼운 보행 흔들림을 적용해 정면, 양쪽 사선, 좌우 측면이 자연스럽게 반복된다. 모션 감소 설정에서는 정면 정지 화면을 유지한다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 레이저건 상단 뷰

- 구현 목적: 중앙을 대칭 조준하는 두 레이저건을 옆면 시점이 아닌 위에서 내려다보는 형태로 표현했다.
- 주요 변경 사항: Retrogun 모델의 전후 기울기를 제거하고 카메라를 모델 상단에 배치했다. 카메라의 위쪽 축을 조정해 총의 길이 방향이 화면에서 유지되며, 기존 `/`, `\` 대칭 조준 각도와 발사 효과는 그대로 적용된다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 승리 티라노 구도와 재질 보정

- 구현 목적: 마지막 승리 화면의 티라노가 지나치게 정면으로 보이는 문제를 줄이고 모델의 존재감과 피부 가독성을 높였다.
- 주요 변경 사항: 승리 티라노를 정면에서 약 18도 옆으로 돌리고 피부 재질의 명도와 장면 노출을 소폭 높였다. 승리 장면 카메라 거리를 16% 줄여 티라노가 기존보다 크게 보이도록 조정했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 스켈레톤 화질과 티라노 회전 연속성 개선

- 구현 목적: 사격 스켈레톤의 확대 시 깨짐과 방향 전환 순간의 끊김을 줄이고 승리 티라노의 점프 동작에 자연스러운 좌우 회전을 추가했다.
- 주요 변경 사항: 3D 캔버스 내부 해상도를 1600×900으로 높이고 안티앨리어싱을 적용했다. 사격 애니메이션을 24fps로 높이고 서버 방향값에 따른 캔버스 순간 반전을 제거했으며, 스켈레톤이 -30도와 +30도 사이를 단일 연속 곡선으로 회전하게 했다. 승리 티라노도 점프 중 같은 좌우 30도 범위를 부드럽게 왕복하며 카메라 조준점을 높여 화면에서 조금 아래에 위치하도록 조정했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 사격 레이저건 좌우 위치 교환

- 구현 목적: 사격 화면에 표시되는 두 레이저건의 좌우 배치를 의도한 구도에 맞췄다.
- 주요 변경 사항: 기존 좌측 레이저건을 우측으로, 기존 우측 레이저건을 좌측으로 이동했다. 총 모델, 팀 색상, 발사 효과와 좌우 반전 구도는 유지했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사와 프로덕션 빌드 및 diff 검사를 수행했다.

## 2026-07-30 - 사격 적중 피드백 강화

- 구현 목적: 약점 명중 순간과 득점 결과를 즉시 인지할 수 있도록 사격 피드백을 강화했다.
- 주요 변경 사항: 총구에서 약점까지 이어지는 레이저와 함께 3중 충격파, 섬광, 뼛가루 파티클, `플레이어 이름 +점수` 팝업을 CSS 일회성 애니메이션으로 추가했다. 약점 위치가 변경될 때 짧은 보간 이동을 적용했다.
- API/Socket.IO/Shared 계약 변경: 없음. 기존 `energy:shotResolved`의 서버 판정 결과와 `energyDelta`를 그대로 사용한다.
- 환경 변수 변경: 없음.
- 검증 결과: `npm run test -w frontend`, `npm run build -w frontend` 통과.

## 2026-07-30 - 모바일 사격 적중 진동

- 구현 목적: 데스크탑을 보며 조준하는 플레이어가 자신의 적중 여부를 휴대폰 촉각으로 즉시 확인할 수 있도록 했다.
- 주요 변경 사항: `energy:fire` 서버 응답이 적중으로 판정되면 연결된 발사 휴대폰에서 진동을 실행한다. 급소 적중은 더 강한 이중 진동 패턴으로 구분하며 Vibration API 미지원 기기에서는 조용히 생략한다.
- API/Socket.IO/Shared 계약 변경: 없음. 기존 서버 권위 `energy:fire` 응답의 `hit`, `energyDelta`를 사용한다.
- 환경 변수 변경: 없음.
- 검증 결과: 프론트 테스트 및 프로덕션 빌드로 검증한다.

## 2026-07-30 - 영점 조정 시간 단축

- 구현 목적: 사격 전 영점 조정 대기 시간을 줄여 게임 흐름을 빠르게 만들었다.
- 주요 변경 사항: 공용 영점 조정 시간을 30초에서 20초로 단축해 데스크탑과 모바일 카운트다운에 동일하게 반영했다.
- API/Socket.IO/Shared 계약 변경: `CHARGING_PRACTICE_DURATION_MS` 값을 `20_000`으로 변경했다.
- 환경 변수 변경: 없음.
- 검증 결과: 전체 테스트와 프로덕션 빌드로 검증한다.

## 2026-07-29 - 개인 MVP 상세 순위표

- 구현 목적: 최종 결과 화면의 개인 MVP 순위를 실제 플레이 기록과 함께 한눈에 비교할 수 있도록 개선했다.
- 주요 변경 사항: 개인 MVP 상위 3명을 순위별 한 줄 표로 표시하고 이름, 땅 판 횟수, 운석 피한 횟수, 에너지 사격 적중 수와 최종 MVP 점수를 함께 노출한다.
- API/Socket.IO/Shared 계약 변경: 없음. 기존 게임 결과의 MVP 순위와 플레이어 통계를 연결해 사용한다.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-29 - 개인 MVP 글씨체 통일

- 구현 목적: 개인 MVP 상세 순위표의 일부 숫자와 항목명이 기본 글꼴로 표시되는 문제를 수정했다.
- 주요 변경 사항: 개인 MVP 제목, 열 제목, 순위, 이름, 플레이 기록과 점수 전체에 팀명과 동일한 `YeogiOttaeJalnan` 글꼴과 굵기를 명시적으로 적용했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사와 프로덕션 빌드 및 diff 검사를 수행했다.

## 2026-07-30 - 사격 약점 개인 득점 표시

- 구현 목적: 새 사격 규칙에 맞춰 현재 약점을 맞힌 플레이어의 성과가 점수판에서 명확히 보이도록 했다.
- 주요 변경 사항: 사격 점수판의 `에너지 기여`와 일반 `명중` 표현을 `개인 점수`, `약점 명중`으로 변경하고 팀 하단 요약도 현재 약점 중심으로 정리했다.
- API/Socket.IO/Shared 계약 변경: 없음. 기존 플레이어 통계와 약점 변경 이벤트를 사용한다.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사, 전체 테스트, 프로덕션 빌드와 diff 검사를 수행했다.

## 2026-07-30 - 발굴 진행 바 상단 고정

- 구현 목적: 화석 발굴 화면의 다음 뼈 진행률을 시선 이동 없이 바로 확인할 수 있도록 배치를 개선했다.
- 주요 변경 사항: 각 팀의 발굴 진행 바를 화면 상단 경계에서 16px 아래에 고정해 기존보다 위쪽에 배치했다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: 타입 검사와 프로덕션 빌드 및 diff 검사를 수행했다.
