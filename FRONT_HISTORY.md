...
## 2026-07-29 - 티라노·레이저건 단일 재질 렌더링 복구

- 구현 목적: 정상 로딩된 3D 모델이 화면에 전혀 그려지지 않는 Three.js 재질 형태 오류를 해결한다.
- 원인: 스켈레톤의 254개 메시가 모두 geometry group 없는 단일 재질인데, 색상 보정 과정에서 재질을 배열로 바꿔 Three.js draw call이 생성되지 않았다.
- 주요 변경 사항:
  - 원본이 단일 재질인 메시에는 복제한 단일 재질을 그대로 할당한다.
  - 원본이 다중 재질인 메시만 복제 재질 배열을 유지한다.
  - 같은 재질 변환 오류가 있던 레이저건에도 동일한 수정을 적용한다.
- API/Socket.IO/Shared 계약 변경: 없음.
- 환경 변수 변경: 없음.
- 검증 결과: `npm run typecheck`, `npm test`(72개), `npm run build`, `git diff --check` 통과.
