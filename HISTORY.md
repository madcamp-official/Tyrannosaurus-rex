...
- 서버: 단순 히트박스(정규화 좌표 반경) 기반 코어/일반 뼈/관절 바깥 판정. 코어는 5초마다
  ...
  초과 시 좀비로 정화 단계 진입, 정화 10초 내 안정도 100 도달 시 정상, 실패 시 영구
  좀비로 확정. 10Hz 배경 틱으로 `trex:transform`/`energy:coreChanged` 방송
  (`server/src/game/charging.ts`, `server/src/game/energy.ts`).
- 라운드 승패 확정: 정상 부활 즉시 승리, 양 팀 모두 좀비면 DRAW, 300초 타임아웃 시 팀
