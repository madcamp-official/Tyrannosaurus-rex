...
팀 간에는 3경기(발굴·다이노런·사격)의 속도와 완성도를 경쟁하지만, 팀 안에서는 발굴과 에너지 주입을 함께 수행하고 다이노런은 개인 완주가 곧 팀 기여가 된다. 개인 점수는 MVP 순위로만 보여주고 팀 승리를 우선한다.
...
| 라운드 제한 시간 | 총 5분 |
| 승리 조건 | 3경기(발굴·다이노런·사격)에서 얻은 점수(타임 보너스·완주·명중률 등)를 합산해 총점이 높은 팀 |
...
- 데스크탑 분할 화면에는 완성될 골격이 점선 실루엣으로만 보인다.
- 데스크탑에 "폰을 봐주세요" 안내가 뜨고, 각 플레이어는 자기 폰에서 개인전으로 다이노런(장애물 러너) 미니게임을 진행한다.
- 제한 시간은 30초. 시간 안에 완주하지 못한 플레이어는 이번 경기에 기여하지 못한다.
- 팀원 한 명이 완주할 때마다 골격 조각이 하나씩 자리에 맞춰진다.
- 분할 화면 양쪽 가장자리에는 완주한 팀원의 이름이 표시된다.
- **점수**: 30초가 지나면 경기가 종료되고, 완주한 인원의 비율(완주 인원 ÷ 팀 인원)과 팀원들의 평균 완주 시간(타임 보너스)을 함께 반영해 점수를 매긴다.
...
- 팀별 결과 표시 뒤, 개인 MVP 1~3위를 표시한다. MVP 점수는 발굴 기여 횟수, 다이노런 완주 여부, 사격 명중 수를 가중 합산해 산정한다.
...
- 탭·흔들기 기반 뼈 발굴, 선착순 승리 판정
- 다이노런(30초 개인 러너) 기반 조립 미니게임과 완주 비율 승리 판정
- 자이로·터치패드 조준과 탭 발사, 단일 공유 스켈레톤 사격
- 서버 권위 히트 판정
- 정상·와이라노 상태(팀별 개별 판정)
- 3판 2선승 최종 승패, 개인 MVP 1~3위, 재경기
- 티라노 이름 및 티꾸 투표
- 최근 20마리 로컬 박물관
...
| 다이노런 조립 | 팀당 동일한 13개 골격 조각, 완주 1회당 1조각 고정 |
| 부활 에너지 | 팀원 데미지 합산 |
...
- 팀원별 발굴 기여도
- 돌과 화석 발견 이펙트
- 필요한 뼈를 먼저 모두 모은 팀에 선착순 승리 표시

#### 다이노런 조립 화면 (경기 2)

- 좌우 팀별 티라노 골격 점선 실루엣 (아직 채워지지 않은 조각은 점선으로만 표시)
- 화면 중앙 "폰을 봐주세요" 안내
- 분할 화면 양쪽 가장자리에 완주한 팀원 이름 나열
- 완주할 때마다 실루엣 조각이 하나씩 채워지는 연출
- 30초 카운트다운
- 종료 시 완주 비율이 높은 팀에 승리 표시
...
- 개인 MVP 1~3위 (발굴 기여·다이노런 완주·사격 명중 가중 합산)
- 티라노 이름
- 티꾸 결과
- 티라노박물관 이동 버튼
...
| 다이노런 | 장애물 회피 조작(탭/스와이프), 30초 카운트다운, 완주 여부 표시 |
...
> 원본 Stan 골격의 254개 세부 메시를 퍼즐용 대형 조각 13개로 다시 묶는다. 조각은 머리뼈, 아래턱, 목뼈, 등뼈, 갈비뼈, 골반, 양팔, 양다리, 꼬리 3구간이며 `desktop-godot/scripts/TrexPuzzleModel.gd`와 1:1로 대응한다.
...
경기 2 시작과 동시에 데스크탑 화면엔 팀별 골격이 점선 실루엣으로만 표시되고, "폰을 봐주세요" 안내가 뜬다. 조각 배치는 더 이상 드래그·회전으로 조작하지 않고, 모바일에서 진행하는 개인전 러너 미니게임의 결과로 자동 결정된다.

#### 진행

1. 경기 2가 시작되면 모든 플레이어의 모바일 화면이 다이노런(장애물 러너)으로 전환된다.
2. 각 플레이어는 자기 폰에서 독립적으로 30초 동안 장애물을 피하며 달린다.
3. 제한 시간 안에 완주하면 서버에 완주를 보고하고, 그 팀의 아직 채워지지 않은 골격 조각 중 정해진 순서의 다음 조각이 즉시 고정된다.
4. 완주하지 못하면(제한 시간 초과 또는 장애물 충돌 3회 등 실패 조건) 그 플레이어는 이번 경기에 조각을 기여하지 못한다.
5. 데스크탑 분할 화면 가장자리에 완주한 팀원 이름이 실시간으로 추가된다.

#### 조각 채움 규칙

- 팀당 조각 수는 발굴과 동일하게 13개로 고정한다.
- 완주 1회당 조각 1개가 고정되며, 완주 인원이 조각 수(13명)보다 많으면 초과분은 조각 채움에 영향을 주지 않고 개인 통계(다이노런 완주 여부)에만 반영한다.
- 완주 인원이 13명보다 적으면 해당 팀은 골격이 끝까지 채워지지 않은 채로 경기가 종료될 수 있다 — 이 경우에도 다음 경기(사격)로는 정상 진행하며, 시각적으로는 남은 조각을 자동으로 채워 넣어 이후 경기가 막히지 않게 한다.

#### 판정

- 30초 타이머가 끝나면 즉시 경기를 종료한다(완주 여부와 무관하게 진행 중인 러너는 중단).
- 팀별 완주 비율 = 그 팀의 완주 인원 ÷ 팀 인원.
- 완주 비율이 더 높은 팀이 경기 2를 승리한다. 동률이면 무승부로 처리하고 3판 2선승 집계에는 반영하지 않는다(§3).
...
- 양 팀이 13개 뼈 조각을 모두 모으면 각자 퍼즐 단계에 진입한다.
...
- 조각별 단일 조작권과 5초 타임아웃
- 위치·각도 정답 판정과 오답 잠금
- 13개 3D 뼈 조각과 Skeleton3D target transform
