...
팀 간에는 속도를 경쟁하지만 팀 안에서는 발굴, 다이노런, 에너지 주입을 함께 수행한다. 개인 점수는 결과 통계로만 보여주고 팀 승리를 우선한다.
...
- 발굴이 끝나면 팀원 전원이 동시에 개인 휴대폰에서 30초 다이노런을 달린다.
- 달리는 공룡 앞으로 장애물이 다가오고, 화면을 탭해 점프로 피한다.
- 장애물 스케줄은 서버가 라운드 시드로 생성해 양 팀이 동일한 코스를 달린다.
- 30초가 끝나면 팀 전체의 장애물 클리어율로 골격 조립 형태를 평가한다.
  잘 달릴수록 완벽한 골격, 못 달릴수록 엉성한 골격이 조립된다.
- 조립 평가 등급(완벽/양호/엉성/누더기)이 3단계의 시작 안정도를 결정한다.
...
- 탭·흔들기 기반 뼈 발굴
- 팀원 전원 동시 30초 다이노런과 클리어율 기반 조립 평가
- 자이로·터치패드 조준과 탭 발사
- 서버 권위 히트 판정
- 정상·와이라노 상태
- 승패·개인 통계·재경기
...
| 다이노런 | 양 팀 동일한 시드 기반 장애물 코스, 클리어율은 팀원 수로 정규화 |
| 부활 에너지 | 팀원 데미지 합산 |
...
- 좌우 팀별 30초 카운트다운
- 팀원별 장애물 클리어 수 실시간 표시
- 종료 시 조립 평가 등급과 함께 완성(또는 엉성한) 골격 표시
