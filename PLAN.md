...
- 부활 에너지와 생체 안정도는 사격 과정의 지표로 사용하되 최종 티라노 형태는 승패로 판정
- 최종 패배 팀은 부활 성공 여부와 관계없이 와이라노가 되고, 승리 팀의 티라노사우루스는 폴짝거리며 뛰어다닌다
- 완성한 티라노를 데스크탑 로컬 `티라노박물관`에 저장
- Node.js 서버를 방·페이즈·입력·다이노런·사격·승패의 유일한 진실 소스로 사용
...
- 승리 팀의 폴짝이는 티라노사우루스와 패배 팀의 와이라노 연출이 기억에 남도록 만든다.
...
초대 링크 접속
  → 게임 제목 + 방 만들기
  → 방 이름·팀별 인원 설정
  → 방 코드·QR·팀 목록(T라노 팀/F라노 팀)
  → 전원 준비 완료 시 자동 시작
  → 경기 1 — 뼈 발굴 (타임 보너스 점수)
  → 경기 2 — 다이노런 조립 (클리어율 점수)
  → 경기 3 — 부활 에너지 사격 (명중률 점수)
  → 3경기 누적 점수 결과 + 개인 MVP
  → 결과·티꾸
  → 티라노박물관
```
...
에너지 품질이 부족하면 단순 패배 문구 대신 "와이라노"가 탄생한다. 결과 화면에는 완성된 티라노 대신 포효하는 티라노 사진에 "와이라노..." 말풍선이 달린 레퍼런스 이미지를 띄워 실패도 웃음 포인트가 되게 한다.

#### 경쟁보다 협동이 먼저 보이는 구조

팀 간에는 3경기(발굴·다이노런·사격)의 속도와 완성도를 경쟁하지만, 팀 안에서는 발굴과 에너지 주입을 함께 수행하고 다이노런은 개인 클리어 수가 곧 팀 기여가 된다. 개인 점수는 MVP 순위로만 보여주고 팀 승리를 우선한다.
...
| 항목 | 규칙 |
|---|---|
| 게임 모드 | 2팀 대전 (T라노 팀 / F라노 팀) |
| 참가 인원 | 팀별 1~10명. 방 생성 시 호스트가 팀별 최대 인원을 지정(시스템 상한 10명) |
| 팀 구성 | 자동 균형 배정 |
| 권장 인원 | 4~6명 |
| 입장 방식 | 4자리 방 코드와 QR |
| 회원가입 | 없음 |
| 게임 시작 | 전원(양 팀 모두) 준비 완료 시 자동 시작. 호스트가 누르는 별도의 시작 버튼 없음 |
| 라운드 제한 시간 | 총 5분 |
| 승리 조건 | 3경기(발굴·다이노런·사격)에서 얻은 점수(타임 보너스·클리어율·명중률 등)를 합산해 총점이 높은 팀 |

팀 인원이 홀수이면 먼저 입장한 팀부터 한 명씩 교대 배정한다. 팀별 목표량은 인원수에 맞춰 보정하여 인원이 적은 팀이 구조적으로 불리하지 않게 한다.

> **구현 가정**: 자동 시작의 최소 조건은 양 팀에 각 1명 이상이 참가하고 전원이 준비 완료 상태일 것. 팀당 0명인 상태로는 자동 시작하지 않는다.

### 2.3 라운드 구성

3경기(발굴 → 다이노런 조립 → 에너지 사격)를 차례로 치르고, 경기마다 팀 점수를 얻는다. 3경기 점수를 모두 합산해 총점이 높은 팀이 최종 승리한다(경기 단위 승패를 가리지 않는 누적 점수제).

#### 경기 1 — 뼈 발굴

- 폰을 흔들어서 흙을 판다.
- 발굴 중 돌, 일반 화석, 황금 뼈가 확률적으로 등장한다.
- 팀에 필요한 뼈를 모두 모으면 완료 시각이 기록되고, 다음 경기로 넘어간다.
- **점수**: 완료까지 걸린 시간을 기준으로 한 타임 보너스(빨리 모을수록 높은 점수). 두 팀 모두 완료하거나 라운드 제한 시간에 도달하면 이 경기의 점수가 확정된다.

#### 경기 2 — 다이노런 조립

- 데스크탑 화면에 "폰을 봐주세요" 안내가 뜨고, 팀의 마지막 뼈가 발굴되는 순간 팀원 전원의 휴대폰에서 다이노런이 동시에 시작된다.
- 30초 동안 달리는 공룡 앞으로 장애물 12개가 서버 스케줄대로 다가오고, 화면을 탭해 점프로 피한다.
- 장애물 스케줄은 라운드 시드로 생성해 양 팀이 완전히 같은 코스를 달린다.
- 30초가 끝나면 팀 전체의 장애물 클리어율(팀 클리어 수 ÷ (장애물 12개 × 팀원 수))로 골격 조립 형태를 평가한다. 골격 자체는 13개 뼈가 모두 조립된 채로 완성되며, 클리어율이 낮을수록 서사적으로 "엉성한" 골격이라는 연출과 등급(완벽/양호/엉성/누더기)이 남는다.
- 이 등급은 경기 3(부활 에너지 사격)의 시작 안정도(40 + 60 × 클리어율)를 결정한다.
- **점수**: 팀 클리어율을 그대로 경기 2 점수로 사용한다.

#### 경기 3 — 부활 에너지 사격

- 완성된 스켈레톤 티라노는 모니터에 단 하나만 표시되고, 두 팀이 같은 티라노를 향해 동시에 사격한다.
- 티라노는 계속 위치를 움직인다.
- **조준 부위가 15초마다 바뀐다**: 머리, 심장 등 코어 부위가 순환하며, 그 순간 활성화된 부위에 맞춰야 높은 판정을 받는다(§6.3 판정 부위).
- 스마트폰 자이로 센서를 기울여 조준하고(대체 입력으로 터치패드) 화면을 탭해 에너지파를 발사한다.
- 팀원별 조준점이 실시간으로 표시되고, 화면 양쪽에는 팀원 닉네임과 각자 명중시킨 횟수가 표시된다.
- 정확한 부위에 에너지를 주입해 부활 게이지와 안정도를 높인다.
- **점수**: 명중률(발사 대비 명중 비율)과 활성 코어 명중 횟수를 기준으로 매긴다.

#### 최종 형태 판정

- 경기 3의 에너지와 안정도는 사격 진행도 및 점수 산정에 사용한다.
- 최종 형태는 3경기 누적 점수의 승패로 확정한다.
- 부활 성공 여부와 관계없이 승리 팀은 밝은 초원 위를 걷는 3D 티라노사우루스, 패배 팀은 와이라노 이미지가 된다.
- 무승부는 승리 팀이 없으므로 양 팀 모두 와이라노가 된다.

#### 최종 결과

- 3경기에서 얻은 점수를 모두 합산해 총점이 높은 팀이 최종 승리한다.
- 승리 팀에는 날이 밝은 하늘 아래 3D 초원 위를 방향 전환하며 걷는 티라노사우루스 연출을, 패배 팀에는 결과 카드 크기에 맞춘 `yranno.png` 기반 "와이라노..." 연출을 띄운다. 부활 성공 여부와 무관하게 누적 점수 승패만 기준으로 한다.
- 팀별 결과 표시 뒤, 개인 MVP 1~3위를 표시한다. MVP 점수는 발굴 기여 횟수, 다이노런 개인 클리어 수, 사격 명중 수를 가중 합산해 산정한다.

> **구현 가정**: 경기별 점수 산식(타임 보너스 곡선, 명중률 가중치 등 구체적인 계수)은 §4 팀별 게임 밸런스에서 값을 확정한 뒤 채운다. 지금은 "무엇을 기준으로 점수를 매기는가"까지만 정한 상태다.

### 2.4 MVP 범위

#### 반드시 구현

- 방 이름·팀별 최대 인원(상한 10명)을 지정하는 방 생성 폼과 QR 모바일 입장
- T라노 팀/F라노 팀 자동 균형 배정과 준비 상태, 전원 준비 완료 시 자동 시작
- 탭·흔들기 기반 뼈 발굴과 경기 1 타임 보너스 점수
- 팀원 전원 동시 30초 다이노런과 클리어율 기반 조립 평가, 경기 2 점수
- 자이로·터치패드 조준과 탭 발사, 단일 공유 스켈레톤 사격
- 서버 권위 히트 판정
- 최종 티라노사우루스·와이라노 상태(누적 점수 승패 판정)
- 3경기 누적 점수 최종 승패, 개인 MVP 1~3위, 재경기
- 티라노 이름 및 티꾸 투표
- 최근 20마리 로컬 박물관
...
경기 1(뼈 발굴)·경기 2(다이노런 조립)·경기 3(부활 에너지 사격)를 차례로 치르고, 경기마다 팀 점수를 얻는다.

- 누가 더 땅을 빨리 팠는지 (경기 1)
- 누가 다이노런을 더 잘 달렸는지 (경기 2)
- 누가 더 티라노를 빨리·정확히 살려냈는지 (경기 3)

- 경기 1: 완료 타임 보너스
- 경기 2: 장애물 클리어율
- 경기 3: 명중률 + 활성 코어 명중 횟수

3경기 점수를 모두 더해 총점이 높은 팀이 최종 승리한다. 경기 단위로 승패를 가리지 않고, 매 경기의 점수를 그대로 누적한다. 점수 산식의 세부 계수는 §2.3, §4를 따른다.

### 티라노사우루스와 와이라노

3경기 누적 점수로 이긴 팀은 티라노사우루스가 되고 결과 화면의 밝은 초원 3D 무대에서 직접 걸어다닌다. 진 팀은 사격 단계의 부활 성공 여부와 관계없이 `yranno.png` 이미지와 "와이라노..." 문구로 표시한다. 무승부일 때는 양 팀 모두 와이라노가 된다.
...
| 뼈 발견 목표 | 팀 인원과 무관하게 핵심 뼈 13종 |
| 뼈 발견 확률 | 팀의 누적 유효 발굴량을 기준으로 계산 |
| 다이노런 조립 | 양 팀 동일한 시드 기반 장애물 코스, 클리어율은 팀원 수로 정규화 (13개 뼈는 항상 완성, 등급만 달라짐) |
| 부활 에너지 | 팀원 데미지 합산 |
...
| 모바일 입력 전송 주기 | 100ms |
| 다이노런 시간 | 30초 |
| 다이노런 장애물 수 | 12개 (양 팀 동일 스케줄) |
| 점프 판정 창 | 장애물 시각 ±450ms |
| 조립 평가 등급 | 클리어율 0.85↑ 완벽 / 0.6↑ 양호 / 0.3↑ 엉성 / 미만 누더기 |
| 충전 시작 안정도 | 40 + 60 × 클리어율 |
| 사격 제한 시간 | 90초 |
| 코어(조준 부위) 회전 주기 | 15초 |
| 발사 쿨다운 | 350ms |
| 조준 전송 빈도 | 최대 30Hz |
...
- 게임 로고와 부제, 방 만들기 버튼
- 방 만들기 클릭 시 방 이름 입력, 팀별 최대 인원 지정(상한 10명)

#### 대기실 (방 상세)

- 4자리 방 코드와 QR
- 참가자 명단과 팀 배정 (T라노 팀 / F라노 팀)
- 팀 이름과 팀 색상
- 준비 상태 표시 — 전원 준비 완료 시 자동으로 게임 시작(별도 시작 버튼 없음)
- 음소거와 전체화면 버튼
...
- 팀원별 발굴 기여도
- 돌과 화석 발견 이펙트
- 필요한 뼈를 먼저 모두 모은 팀의 완료 시각 표시(타임 보너스로 경기 1 점수에 반영)

#### 다이노런 조립 화면 (경기 2)

- 화면 중앙 "폰을 봐주세요" 안내
- 좌우 팀별 30초 카운트다운
- 팀원별 장애물 클리어 수 실시간 표시(분할 화면 가장자리에 나열)
- 종료 시 조립 평가 등급과 함께 완성(또는 엉성한) 골격 표시, 경기 2 점수(클리어율) 표시

#### 에너지 사격 화면 (경기 3)

- 모니터 중앙에 스켈레톤 티라노 단 하나만 표시 (좌우 분할 아님, 두 팀이 같은 대상을 동시 조준)
- 계속 위치가 바뀌는 골격
- 플레이어 색상의 크로스헤어 (양 팀 전원)
- 화면 양쪽에 팀별 팀원 닉네임과 개인 명중 수
- 부활 에너지와 생체 안정도
- 남은 시간
- 와이라노 위험 경고
...
- 3경기 누적 점수 결과 — 승리 팀은 밝은 초원 위를 걷는 3D 티라노사우루스, 패배 팀은 `yranno.png` 기반 "와이라노..." 연출
- 팀별 최종 티라노사우루스 또는 와이라노 상태 표시
- 경기별(발굴·다이노런·사격) 획득 점수 요약
- 개인 MVP 1~3위 (발굴 기여·다이노런 클리어 수·사격 명중 가중 합산)
- 티라노 이름
- 티꾸 결과
- 티라노박물관 이동 버튼
...
| 페이즈 | 모바일 컨트롤 |
|---|---|
| 대기실 | 닉네임, 팀 표시, 준비 상태(누르면 전원 완료 시 자동 시작) |
| 발굴 | 센서 권한, 흔들기 상태, 큰 발굴 버튼 |
| 다이노런 | 달리는 공룡 뷰, 점프 버튼(화면 전체 탭), 남은 시간, 내 클리어 수 |
...
> 원본 Stan 골격의 254개 세부 메시를 조립용 대형 조각 13개로 다시 묶는다. 조각은 머리뼈, 아래턱, 목뼈, 등뼈, 갈비뼈, 골반, 양팔, 양다리, 꼬리 3구간이며 `desktop-godot/scripts/TrexPuzzleModel.gd`와 1:1로 대응한다.
...
경기 2 시작과 동시에 데스크탑 화면엔 "폰을 봐주세요" 안내가 뜬다. 조각 배치는 더 이상 드래그·회전으로 조작하지 않고, 모바일에서 진행하는 다이노런(장애물 러너) 미니게임의 결과로 자동 결정된다.

#### 진행

1. 팀의 마지막 뼈가 발굴되는 순간 팀원 전원의 휴대폰에서 다이노런이 동시에 시작된다.
2. 30초 동안 달리는 공룡 앞으로 장애물 12개가 서버 스케줄대로 다가온다.
3. 화면을 탭하면 점프한다. 점프 입력은 서버로 전송된다.
4. 서버는 점프 수신 시각이 어떤 장애물의 판정 창(±450ms) 안인지 판정해 클리어를 기록한다.
5. 30초가 끝나면 서버가 팀 클리어율로 조립 형태를 평가하고 사격 단계로 전환한다.

#### 공정성 규칙

- 장애물 스케줄은 라운드 시드로 생성해 양 팀이 완전히 같은 코스를 달린다.
- 팀 클리어율 = 팀 전체 클리어 수 ÷ (장애물 12개 × 팀원 수) — 인원과 무관하게 공정하다.
- 한 장애물은 플레이어당 한 번만 클리어로 인정한다. 점프 연타는 rate limit으로 막는다.

#### 조립 평가와 경기 2 점수

- 클리어율 0.85 이상 완벽, 0.6 이상 양호, 0.3 이상 엉성, 미만 누더기.
- 골격은 13개 뼈가 모두 조립된 상태로 완성되지만, 평가 등급이 낮을수록
  엉성한 형태라는 서사를 결과 화면과 통계에 남긴다.
- 사격 단계의 시작 안정도 = 40 + 60 × 클리어율. 못 달린 팀은 낮은 안정도로 시작한다.
- 팀 클리어율을 그대로 경기 2 점수로 사용해 §3 누적 점수제에 반영한다.

### 6.3 부활 에너지 사격

#### 티라노 움직임

- 모니터에는 스켈레톤 티라노가 단 하나만 표시되며, 두 팀이 같은 개체를 동시에 조준·사격한다(팀별 분할 무대가 아니다).
- 서버가 공용 티라노의 기준 위치와 방향을 10Hz로 갱신한다.
- 티라노는 화면 중앙 무대 안에서 좌우 이동하고 간헐적으로 자세를 바꾼다.
...
핵심 코어는 15초마다 다른 부위(머리 ↔ 심장 ↔ 척추 등)로 이동하여 한곳만 연타할 수 없게 한다. 그 순간 비활성 상태인 코어에 맞으면 위 표의 "핵심 코어" 대신 "일반 뼈" 판정을 받는다.
...
- 제한 시간이 끝나면 입력을 잠그고 사격 점수를 확정한다. 최종 형태는 3경기 누적 점수 승패 확정 뒤 결정한다.
...
└───────────────┬────────────────────────────────────────────┘
                ...
                ▼
┌──────────────── Node.js 권위 서버 ────────────────────────┐
│ 방·팀·페이즈·입력 제한·다이노런 판정·사격 판정·승패 계산   │
└───────────────▲────────────────────────────────────────────┘
                │ Socket.IO
┌───────────────┴──── 모바일 브라우저 × 2~6 ────────────────┐
...
- 현재 팀별 페이즈와 제한 시간
- 유효한 발굴 횟수와 뼈 획득
- 다이노런 장애물 스케줄과 점프 클리어 판정
- 크로스헤어 좌표와 발사 쿨다운
- 티라노 위치, 히트박스, 명중 부위
- 에너지, 안정도, 와이라노 상태와 최종 승리
...
| `REVIVAL_RESULT` | 승패 기반 최종 형태 | 승자는 밝은 초원 육체 티라노 3D 걷기 연출, 패자는 와이라노 이미지 연출 |
...
- 대형 뼈 조각 13개는 각각 독립 `Node3D`와 고유 `boneId`를 갖는다.
- 다이노런 진행 중에는 발굴로 드러난 뼈들이 흩어진 상태로 대기한다.
- `dino:finished`가 오면 13개 조각이 차례로 해부학 위치로 스냅되는 완성 연출을 재생한다.
- 조립 등급이 낮으면(엉성/누더기) 스냅 후 일부 조각을 미세하게 기울여 엉성함을 표현한다(P1).
- 러너 자체는 모바일 2D UI다. Godot은 조립 결과만 그린다.

### 12.4 스켈레톤 티라노

- 결과 화면은 스켈레톤이 아닌 육체 티라노 3D 모델을 사용한다. 승자는 밝은 초원 위를 실제 3D 모델로 걸으며 이동하고 패자는 `yranno.png` 이미지로 표시한다.
- 발굴한 13개 게임 조각과 원본의 254개 시각용 세부 메시를 분리한다.
- 다이노런 평가 완료 시 조각 오브젝트를 리그 위치로 흡수하는 연출 후 완성 리그로 전환한다.
...
- 승리: 스켈레톤에서 육체 티라노사우루스로 전환한 뒤 밝은 초원 3D 무대 위를 걸어다닌다.
- 패배: 부활 성공 여부와 무관하게 `yranno.png` 이미지와 "와이라노..." 문구로 표시한다.
...
Team B:        EXCAVATION ───────→ ASSEMBLY → CHARGING → REVIVED
```

`CHARGING` 제한 시간(90초) 안에 부활 게이지 100%를 채우면 `NORMAL`로, 못 채우면 `YRANNO`로 `REVIVED`에 도달한다 — 두 경우 모두 되돌릴 수 없다.
...
- 발견한 뼈와 발굴 포인트
- 다이노런 장애물 스케줄·클리어 현황·조립 평가
- 부활 에너지와 안정도
- 승패 기반 티라노사우루스·와이라노 상태
- 단계별 완료 시각
...
type PublicPlayer = {
  ...
  stats: {
    ...
    dinoCleared: number;      // 다이노런에서 클리어한 장애물 수
    shots: number;
    hits: number;
    coreHits: number;
...
```ts
type DinoRunState = {
  obstacleOffsetsMs: number[];             // 시드 생성, 양 팀 동일, phase 시작 기준 오프셋
  clearedByPlayer: Record<PlayerId, number[]>; // 플레이어별 클리어한 장애물 index
  performance: number | null;              // 0~1, 30초 종료 시 확정
  grade: "PERFECT" | "GOOD" | "CLUMSY" | "MESSY" | null;
};
```

판정은 전적으로 서버가 한다. 클라이언트는 점프 입력만 보내고 점수를 자기
신고하지 않는다.
...
```ts
type DinoJumpRequest = {
  requestId: RequestId;
  seq: number;
  clientTime: number;
};

type DinoJumpResponse = {
  cleared: boolean;            // 이번 점프로 장애물을 클리어했는가
  obstacleIndex: number | null;
  clearedCount: number;        // 이 플레이어의 누적 클리어 수
};
```

처리:

- ASSEMBLY(다이노런) 페이즈에서만 허용, 초당 3회 rate limit
- 서버 수신 시각 기준으로 `phaseStartedAt + obstacleOffsetsMs[i] ± 450ms` 창에
  들어오고 아직 그 플레이어가 클리어하지 않은 장애물이면 클리어로 기록
- 클리어 시 팀·데스크탑에 `dino:progress`를 브로드캐스트
...
| `dino:started` | 팀, 장애물 오프셋 목록, 종료 시각 | 방 전체 |
| `dino:progress` | 팀, 플레이어, 장애물 index, 누적 클리어 수 | 방 전체 |
| `dino:finished` | 팀, 클리어율, 조립 등급, 시작 안정도 | 방 전체 |
...
- 연결 해제된 플레이어의 다이노런은 그 시점까지의 클리어 수로만 평가에 반영한다.
...
- 가짜 플레이어 10명이 입장해도 방 상태가 일치한다.

### Day 2 — 뼈 발굴

#### 목표

모든 지원 기기에서 팀 발굴을 수행하고 13종 뼈 획득까지 진행한다.
...
- 양 팀이 13개 뼈 조각을 모두 모으면 각자 다이노런 단계에 진입한다.
...
팀원 전원이 동시에 30초 다이노런을 달리고 서버 판정 클리어율로 골격 조립을 평가한다.

#### 작업 항목

- 시드 기반 장애물 스케줄 생성(양 팀 동일)
- 모바일 러너 화면: 달리는 공룡, 다가오는 장애물, 탭 점프
- `dino:jump` 서버 판정(±450ms 창, 플레이어당 장애물 1회)
- 30초 종료 시 클리어율 → 조립 등급 → 시작 안정도 계산
- 13개 뼈 조각 완성 스냅과 `PUZZLE_STATE` 브리지 전송
...
- 양 팀이 같은 장애물 스케줄을 받는다.
- 판정 창 밖 점프와 중복 점프는 클리어로 인정되지 않는다.
- 30초 뒤 두 팀 모두 자동으로 사격 단계에 진입한다.
- 클리어율에 따라 시작 안정도가 40~100으로 달라진다.
...
- 10명 30Hz 입력에서도 서버와 데스크탑이 안정적으로 동작한다.

### Day 5 — 에너지 사격과 부활

#### 목표

서버 권위 사격 판정부터 누적 점수 승패 기반 티라노사우루스·와이라노 결과까지 핵심 승패 루프를 완성한다.
...
- 승리 티라노사우루스와 패배 와이라노 탄생을 각각 재현한다.
- 같은 shot ID가 점수를 두 번 변경하지 않는다.
- 서버 판정 결과와 Godot 시각 효과가 일치한다.
- 3경기 누적 점수가 높은 팀만 한 번 승리 처리된다.
...
- 티라노 이름 후보와 투표
- 티꾸 카테고리 투표
- 승리 티라노 3D 점프 연출과 패배 와이라노 3D 연출 노출
...
- 발굴 입력 속도 제한
- 뼈 발견 순서와 이벤트 시드
- 다이노런 점프 판정 창과 중복 클리어 방지
- 다이노런 30초 종료 시 등급·시작 안정도 계산
- 사격 쿨다운과 중복 요청
- 부위별 에너지·안정도 계산
- 승패 기반 티라노사우루스·와이라노 상태 전환
- 제한 시간 동점 처리
...
- 여러 플레이어의 동시 다이노런 점프
- 다이노런 중 연결 해제 시 부분 클리어 평가
- 여러 플레이어의 동시 사격
- 승패 확정 중 결과 이벤트 중복 수신
...
| 다이노런 네트워크 지연 | 넉넉한 판정 창(±450ms)과 서버 수신 시각 기준 판정 |
...
- [ ] 양 팀이 같은 장애물 스케줄을 받는다.
- [ ] 판정 창 밖 점프와 같은 장애물 중복 점프가 클리어로 인정되지 않는다.
- [ ] 30초 종료 시 두 팀 모두 자동으로 사격 단계에 진입한다.
- [ ] 클리어율에 따라 조립 등급과 시작 안정도가 달라진다.
- [ ] 종료 시 Godot에서 13개 조각이 완성 위치로 스냅된다.
...
- [ ] 승리 티라노사우루스·패배 와이라노 결과가 각각 재현된다.
- [ ] 승자가 한 번만 확정된다.
...
- [ ] 다이노런 중 연결 해제된 플레이어는 그때까지의 클리어만 평가에 반영된다.
...
- 사격 시작 시 서버가 공유 스켈레톤 위에 하나의 활성 약점을 표시한다.
- 양 팀 플레이어 중 현재 활성 약점을 정확히 맞힌 플레이어만 개인 점수와 팀 에너지를 얻는다.
- 스켈레톤 몸체 또는 이전 약점 위치를 맞힌 사격에는 점수를 주지 않는다.
- 유효 명중 직후 서버는 현재 부위를 제외한 다른 코어를 다음 약점으로 선택하고 양 팀 화면에 동시에 전파한다.
- 약점 위치와 득점 판정은 Backend가 유일한 권위로 관리한다.

- 기존 코드가 없는 현재 저장소에서 새로 구현한다.
- 게임은 개인전 없이 2팀 대전으로 집중한다.
- 최대 인원은 팀당 5명, 전체 10명이다.
...
- 와이라노는 되돌릴 수 없는 최종 결과이며, 단순 패배 화면 대신 결과 화면의 레퍼런스 이미지로 웃음 포인트를 만든다.
