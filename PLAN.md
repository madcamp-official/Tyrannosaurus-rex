...
- 부활 게이지 100%와 안정도 목표치를 먼저 충족한 팀이 승리한다.
- 게이지만 가득 찼지만 안정도가 부족하면 좀비 티라노가 탄생한다.
- 좀비 티라노가 탄생한 팀은 10초간 정화 사격 기회를 얻는다.
- 정화에 성공하면 정상 부활, 실패하면 상대 팀이 먼저 정상 부활하거나 시간이 끝날 때까지 게임을 계속한다.
...
에너지가 100%에 도달한 순간 안정도가 70% 미만이면 좀비 티라노가 탄생한다.

- 해당 팀의 에너지는 100%로 유지한다.
- 10초 동안 약점에 정화 에너지를 맞혀 안정도를 70%까지 올려야 한다.
- 정화 중 상대 팀은 계속 정상적으로 플레이한다.
- 정화 실패 시 에너지가 70%, 안정도가 40%로 감소한 뒤 다시 충전 단계로 돌아간다.

### 제한 시간 종료

정상 부활 팀이 없으면 다음 순서로 승자를 결정한다.
...
좀비 티라노는 기본적으로 해진 장식과 녹색 오라를 사용한다. 정상 부활에 성공하면 같은 아이템이 깨끗한 버전으로 표시된다.
...
- 우승 팀이 결과 화면에서 후보 이름 세 개 중 투표한다.
- 후보는 `티`가 포함된 짧은 이름 조합으로 서버가 생성한다.
- 기본 후보 예: 티꾸, 티롱이, 티렉뚜
...
`godot/`은 원본 프로젝트이고 `client/public/godot/`은 빌드 산출물이다. 원본 에셋이나 GDScript를 `client/public`에서 직접 수정하지 않는다.
...
# Godot Web Export 후 client/public/godot에 복사
npm run build:godot
...
```bash
godot --headless --path godot --export-release Web ../client/public/godot/index.html
```
...
| `REVIVAL_RESULT` | 정상·좀비·정화 결과 | 시네마틱 재생 |
...
- 뼈 12개는 각각 독립 `Node3D`와 고유 `boneId`를 갖는다.
...
- 좀비 부활: 녹색 역광 → 불완전 조직 → 비대칭 자세 → 경고 연출
- 조직 생성은 실제 메시 생성이 아니라 dissolve 셰이더의 threshold 애니메이션으로 표현한다.
- 정화 성공 시 좀비 머티리얼을 정상 머티리얼로 블렌딩한다.
...
type RevivalForm = "NONE" | "NORMAL" | "ZOMBIE";
...
type TrexTransformEvent = {
  ...
  rotationDeg: number;
  facing: "LEFT" | "RIGHT";
  poseId: "IDLE" | "WALK" | "ROAR" | "HIT" | "REVIVE" | "ZOMBIE";
  effectiveAt: number;
};
...
- acknowledgement 래퍼와 요청 ID 중복 방지 캐시
- 방 생성, QR 입장, 팀 자동 배정
- 데스크탑·모바일 로비
- Cloudflare Tunnel HTTPS 구성
- 2~6명 시뮬레이션 클라이언트
- Godot 4 Compatibility 프로젝트와 Web Export preset 생성
- 빈 Godot Web 빌드를 React iframe에 임베드
- `GODOT_READY`와 `FULL_SNAPSHOT` 브리지 왕복 검증

완료 기준: 실기기 2대가 HTTPS QR로 입장하고 같은 팀 상태를 보며, React 버튼으로 iframe 속 Godot 장면을 변경할 수 있다.
...
- iOS 센서 권한
- 100ms 입력 집계와 서버 제한
- `excavate:input` sequence와 token bucket 적용
- 12종 뼈 획득
- 팀별 발굴 화면과 기여도
- 돌·화석·황금 뼈 이벤트
- Godot 발굴 무대, 지층, 뼈 발견 오브젝트 풀
- `BONE_DISCOVERED` 브리지 연출 연결

완료 기준: iOS와 Android에서 발굴해 12개 뼈를 모두 모으고 자동으로 퍼즐 단계에 진입한다.
...
- 조각 선택·이동·회전·배치
- 조각 조작권과 타임아웃
- claim token 발급·마스킹·만료 처리
- 위치·각도 서버 판정
- 오답 잠금과 진행률
- 12개 3D 뼈와 Skeleton3D 목표 transform 제작
- `PUZZLE_STATE`를 3D 조각 이동·회전·스냅으로 표현

완료 기준: 팀원 2명이 동시에 서로 다른 조각을 조작하고 서버에서 정답 처리된 12개 조각이 Godot 골격에 정확히 스냅된다.
...
- 터치패드 폴백
- 최대 30Hz 좌표 전송
- 오래된 조준 좌표 무효화와 발사 요청 멱등성
- 팀별 크로스헤어 표시
- Godot CanvasLayer 크로스헤어와 서버 transform 보간
- 시연 PC의 WebGL 성능과 초기 다운로드 크기 측정

완료 기준: 모든 기기에서 터치패드가 동작하고, 지원 기기에서 자이로로 화면 주요 영역을 안정적으로 조준한다.
...
- 10초 정화 단계
- 시간 종료와 승패 판정
- Godot 레이저·피격·부활·좀비 시네마틱
- AnimationTree와 dissolve 셰이더
- 서버 결과와 애니메이션 완료의 분리 검증

완료 기준: 정상 부활, 좀비 탄생, 정화 성공, 정화 실패를 각각 재현하는 자동 테스트가 통과한다.
...
- API 버전과 Godot asset 버전 표시
- Godot 장식 소켓과 `DecorationCatalog`
- 3D 박물관 전시대와 거리별 활성화
- Godot Web 빌드 자동 export 스크립트

완료 기준: 완성한 티라노를 꾸며 박물관에 저장하고 새로고침 후 다시 볼 수 있다.

### Day 7 — 통합 테스트와 시연

- 실기기 4~6대 전체 흐름 테스트
- 캠퍼스 Wi-Fi, 터널, 핫스팟 검증
- 권한 거절, 화면 회전, 연결 해제 테스트
- 부하·프레임·네트워크 프로파일링
- 밸런스 조정
- 시연 및 장애 대응 체크리스트
- 기능 동결

완료 기준: 실기기 6대로 세 판 연속 완주하고 자이로 실패 시 터치패드로 즉시 전환할 수 있다.
...
- 좀비 티라노도 실패 화면으로 끝내지 않고 정화로 역전할 기회를 제공한다.
