...
- 와이라노는 되돌릴 수 없는 최종 결과이며, 결과 화면의 레퍼런스 이미지로 웃음 포인트가 되게 한다
...
초대 링크 접속
  ...
  → 경기 3 — 부활 에너지 사격 (명중률 점수, 정상·와이라노 판정은 별개)
  → 3경기 누적 점수 결과 + 개인 MVP
  → 결과·티꾸
  → 티라노박물관
```
...
- 경기 3에서 제한 시간 내 에너지 100%를 채운 팀에는 정상 부활이 확정된다.
- 제한 시간 내 채우지 못한 팀에는 와이라노가 탄생한다.
- 부활 판정은 팀별 개별 판정이며, 경기 3의 점수(명중률)와는 별개 기준이다. 두 팀 모두 정상 부활에 도달할 수도, 두 팀 모두 와이라노가 될 수도 있다.

#### 최종 결과

- 3경기에서 얻은 점수를 모두 합산해 총점이 높은 팀이 최종 승리한다.
- 승리 팀에는 신나하는 티라노 모습을, 패배 팀에는 "와이라노..." 문구가 붙은 티라노 사진을 결과 화면에 띄운다. 이 승패 연출은 경기 3의 개별 부활 판정과 무관하게 누적 점수 결과를 기준으로 한다.
...
이 판정은 팀별로 독립적으로 이루어지며, 누적 점수 결과와는 별개다 — 최종 결과 화면의 승리/패배 연출은 3경기 총점을 기준으로 하고, 와이라노 여부는 경기 3 안에서 각 팀의 부활 상태를 나타낼 뿐이다.
...
- 3경기 누적 점수 결과 — 승리 팀은 신나하는 티라노, 패배 팀은 "와이라노..." 문구와 티라노 사진
- 팀별 탄생한 정상 또는 와이라노 상태 표시
...
서버 권위 사격 판정부터 정상·와이라노 결과까지 핵심 승패 루프를 완성한다.
...
- 에너지와 안정도
- 핵심 코어 이동
- 정상 부활·와이라노 판정 (제한 시간 내 에너지 100% 여부로 확정, 되돌릴 수 없음)
- 시간 종료와 동점 규칙
