...
- 부활 에너지와 생체 안정도를 분리한 정상·좀비 티라노 판정
- 좀비 티라노가 실패로 즉시 종료되지 않고 정화 사격으로 역전 가능
...
- 정상·좀비 부활과 티꾸로 게임 결과 자체가 기억에 남도록 만든다.
...
에너지 품질이 부족하면 단순 패배 문구 대신 좀비 티라노가 탄생한다. 실패 상태에도 강한 연출과 정화 기회를 주어 웃음과 역전을 만든다.
...
서버 권위 사격 판정부터 정상·좀비·정화 결과까지 핵심 승패 루프를 완성한다.
