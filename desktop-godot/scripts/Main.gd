...
func _build_camera() -> void:
	...
	# 기존 탑다운 구도와 화면 점유율은 유지하되, Z축으로 1.6m만 물려 약 6도 사선에서
	# 내려다보게 한다. 구덩이의 가까운 면과 먼 면이 아주 조금 구분되어 깊이가 읽히면서도
	# 팀별 무대의 중앙 정렬과 좌우 대칭은 흐트러지지 않는다.
	camera.position = Vector3(0, 14.9, 1.6)
	camera.fov = 68.0
	camera.near = 0.1
	camera.far = 100.0
	add_child(camera)
