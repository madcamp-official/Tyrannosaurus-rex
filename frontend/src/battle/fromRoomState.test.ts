...
function makeTeam(overrides: Partial<TeamState> & { id: "A" | "B" }): TeamState {
  return {
    ...
    phaseEndsAt: 90_000,
    playerIds: [],
    excavation: { points: 0, nextBoneAt: 60, discoveredBoneIds: [], fossils: 0, efficiencyMultiplier: 1, debuffEndsAt: null },
...
describe("battleStateFromRoom", () => {
  ...
  it("converts server data into BattleState — core label, facing sign, and team aggregates", () => {
    ...
    expect(battle!.trex).toEqual({ x: 0.4, facing: -1, corePos: [0.42, 0.46] });
