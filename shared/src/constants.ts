...
// 초반 5초·후반 5초가 통째로 비어 있다는 피드백에 따라 초반은 2초만 쉬고, 후반은
// 라운드가 끝나기(60초) 직전까지 거의 쉬지 않고 떨어지도록 좁혔다. FALL_MS(941)+
// COLLISION_GRACE_MS(150)보다 넉넉히 여유를 두어 마지막 오브젝트도 라운드가 끝나기 전에
// 판정까지 끝난다.
export const SKY_OBJECT_MIN_OFFSET_MS = 2_000;
export const SKY_OBJECT_MAX_OFFSET_MS = 59_000;
...
// "동시에 최소 3개" 보장(§규정)이 깨져 0.75로 다시 낮췄다 — 후반이 좀 더 빨라도 된다는
// 피드백에 따라 0.68로 다시 낮춰 초반 대비 후반 밀도를 조금 더 올렸다("전반부가 너무 비지
// 않고 후반부가 너무 몰리지 않는다"는 균형 자체는 여전히 유지하는 선에서).
export const SKY_OBJECT_DENSITY_CURVE_EXPONENT = 0.68;
...
export const FINAL_STAGE_STUN_MS = 2_000;
...
export const ENERGY_HIT_CHASE = 2;
export const ENERGY_HIT_FINAL_CORE = 5;
